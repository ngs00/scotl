...
def get_fvec(form, elem_attrs):
    ...
    return form_vec.numpy()


def get_crystal_graph(struct, elem_attrs, rbf_means, idx, fvec, target, atomic_cutoff=4.0):
    try:
        ...
        fvec = torch.tensor(fvec, dtype=torch.float).view(1, -1)
        target = torch.tensor(target, dtype=torch.float).view(1, 1)
        crystal_graph = Data(x=atom_feats, edge_index=bonds, edge_attr=bond_feats, idx=idx, fvec=fvec, y=target)
